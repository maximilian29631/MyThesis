...
\begin{table}[!hbt]
	...
	\caption{Settings for the PVT runs.}
			\begin{tabularx}{\textwidth}{RLLLLLL}\hline \hline
...
The beta cuts are the energy conditions imposed on the implant detector when the coincidence was imposed.  
Thus, sets 1 and 2 were not used in the final analysis, but are listed here for completeness's sake.
...
These are the energy cuts imposed on the four gamma detectors when the coincidence was imposed.  
...
The lower beta cut was chosen to exclude any potential pile-up from this region. 
...
\begin{figure}
	...
	\caption{A sketch of the pile-up events.
		 A $^{20}$F decay happens at time $\textit{t1}$.
		 The green electron is detected in the implant detector.
		 The black arrow (the gamma ray) is not.
		 Then, later at time $\textit{t2}$, another $^{20}$F decays.
		 $\textit{t2}$ is within the pile-up window of $\textit{t1}$.
		 Both electrons energies pile-up and are added together.
		 However, the time stamp recorded by the DAQ is still $\textit{t1}$.
		 The gamma ray from event $\textit{t2}$ can be detected in a gamma detector at time $\textit{t2}$.
...
This method was chosen to keep the result independent of the binning of the decay curves.  
...
In the gamma-beta energy 2D plot (figure \ref{fig:2DGraph}), it can be seen that there is no background events aside from the 511 keV region.
...
Moving the lower beta cut also affects the dead time correction.
...
%\begin{center}
	\begin{table}[!hbt]
			...
			Right & 10.9987 & 0.0095 \\ 
			      &		& 	 \\
			Mean & 10.9999 & 0.0044 \\ \hline \hline
			\end{tabular}
			\label{tab:PVTTable}
	\end{table}
%\end{center}
...
\begin{table}[!hbt]
	...
	\label{tab:err-budget}
		...
		Binning & -0.30 & 0.30 \\
			&	&	\\
		Total systematic & 1.17 & 2.89 \\ \hline \hline 
		\end{tabular}
	\label{tab:SysTable}
\end{table}
%\end{center}
...
The half-life is important for the calculation of the Gamow-Teller matrix element form factor $c_{1h}$.
The value $0.253 \pm 0.004$ comes from a calculation where the $log ft$ value of $^{20}$F was $4.9753$ \cite{Min11}.
The $log ft$ value was averaged with the $log ft$ value of $^{20}$Na, and have the difference of the two values was taken as an error bar.
Taking the $f$ value from Genz \cite{Gen76}, the $log ft$ value using the current half-life result is $4.9710$. 
This would bring the average value of the $ log ft$ values down from $4.983$ to $4.980$.
This would then move the value of $c_{1h}$ to $0.254$, which is still within the error bar.
The new value of the half-life therefor has a negligible effect on the extraction of the Fierz term.
