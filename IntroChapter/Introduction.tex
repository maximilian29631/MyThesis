...
\end{equation}
with $^{A}_{Z}P$ being the parent nucleus, $^{A}_{Z \pm 1}D$ being the daughter nucleus, $e^{\mp}$ is the outgoing electron or positron, and $\nu$ is an outgoing antineutrino or neutrino.
This is an electron antineutrino in the case of beta$^{-}$ decay, and an electron neutrino in the case of beta$^{+}$ decay. 
...
This process emits a $W^{-}$ boson that decays into an electron and an electron antineutrino.
...
The decay rate function, $W(E,\Omega)$, is \cite{Jack57} %shown in equation \ref{eq:matrixelement} \cite{Gon19}
\begin{equation}
...
\end{equation}
where $M_{F}$ is the Fermi matrix element, $C_{V}$ and $C_{V}'$ are vector coupling constants, $C_{A}$ the axial vector coupling constant, and $|\rho|$ is the ratio of the Gamow-Teller matrix element to the Fermi matrix element times $C_{A}/C_{V}$.
...
\end{equation}
where $\alpha$ is the fine structure constant, $Z$ the atomic number of the daughter nucleus, and $Re()$ gives the real part of an imaginary number \cite{Jack57}.
...
The Fierz term in Fermi decays have been measured using super-allowed beta decays.
...
\begin{figure}[!htb]
        ...
        \caption{The energy spectrum of the ultra-cold neutron measurement \cite{Hic17}.
		 This spectrum was measured using a spectrometer. }
        \label{fig:ucnabeta}
\end{figure}
...
In the case of a pure Gamow-Teller transition, the Fierz term in equation \ref{eq:bwrittenout} can be re-written as %equation \ref{eq:bgt}
